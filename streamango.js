function decrypt(argcode, argbytes) {
  ...
  d = function(_0x5ecd00, _0x184b8d) {
    var _0x388cca = {
      'WSNWx': _0x5029('0x0'),
  ...
  return d(argcode, argbytes);
}
try {
  var match = dom.window.document.body.innerHTML.match(/:(d\([^)]*\))/);
  if (match != null) {
    match = match[1].slice(1);
    var found = Function("return arguments[0]" + match)(decrypt);
    if (options.save)
      fs.writeFileSync(options.save, found);
  } else {
    if (options.save)
